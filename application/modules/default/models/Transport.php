<?php
class Default_Model_Transport extends Zend_Db{
protected $_table_name = "transport";
	...
	public function Transport_listall()
	{
	...
	public function Transport_insert($name,$name_en = ""){
		if($name ){
			//$name =  $this->db->quote($name,STRING);
			//$name_en = $this->db->quote($name_en,STRING);
			if($this->Transport_checkExist($name) == 0){
				$data = array(
						'name'=> $name,
						'name_en' => $name_en);
	...
	public function Transport_getById($id){
		if($id){
			$select = $this->db->select()
					->from($this->_table_name)
					->where("id =?",$id);
