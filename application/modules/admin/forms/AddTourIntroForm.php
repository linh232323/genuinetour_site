...
class Admin_Form_AddTourIntroForm extends Zend_Form {
...
protected $_outward_transport;
	...
	public function init()
	{
		...
		$_name = $this->createElement("text","name")
				->setAttrib("class", "text-input small-input")
				->setLabel("Tên Tour(tiếng Việt)")
				->setRequired(true)
				->addValidator('NotEmpty', true)
				->addErrorMessage('Cần nhập tên Tour.');
		$this->addElement($_name);
		
		$_name_en = $this->createElement("text", "name_en")
				->setAttrib("class", "text-input small-input")
				->setLabel("Tên Tour(English)")
				->setRequired(true)
				->addValidator('NotEmpty', true)
				->addErrorMessage('Cần nhập tên Tour.');
		...
		$_during = $this->createElement("select", "during")
				->setAttrib("class", "text-input small-input")
				->setLabel("Thời gian Tour(tiếng Việt)")
				->setRequired(true)
				->addValidator('NotEmpty', true)
				->addErrorMessage('Cần nhập Thời gian Tour.');
		for($i = 1;$i<=30;$i+= 0.5){
			$name ="";
			if($i == floor($i)){
				$name =  "$i Ngày";
		...
		$_during_en = $this->createElement("select", "during_en")
				->setAttrib("class", "text-input small-input")
				->setLabel("Thời gian Tour(English)")
				->setRequired(true)
				->addValidator('NotEmpty', true)
				->addErrorMessage('Cần nhập Thời gian Tour.');
		for($i = 1;$i<=30;$i+= 0.5){
		...
		$_post_id_intro = $this->createElement("textarea", "post_intro")
				...
				->setLabel("Thông tin Tour(Viet Nam)")
				->setRequired(true)
				->addValidator('NotEmpty', true)
				->addErrorMessage('Cần nhập tên Tour.');
		...
		$_post_id_intro_en->setRequired(false);
		...
		$_appendix_en->setRequired(false);
